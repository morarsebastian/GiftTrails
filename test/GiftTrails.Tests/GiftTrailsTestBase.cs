...
namespace GiftTrails.Tests
{
    public abstract class GiftTrailsTestBase : AbpIntegratedTestBase<GiftTrailsTestModule>
    {
        ...
        protected void UsingDbContext(Action<GiftTrailsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GiftTrailsDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GiftTrailsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GiftTrailsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GiftTrailsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GiftTrailsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GiftTrailsDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GiftTrailsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GiftTrailsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GiftTrailsDbContext, Task<T>> func)
        {
            T result;
